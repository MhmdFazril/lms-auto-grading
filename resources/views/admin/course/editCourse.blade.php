...
@section('content')
    <div class="mx-3 my-6 md:mx-auto md:max-w-7xl">
        <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] p-6">
            ...
            <form action="{{ route('course.update', ['course' => $course->id]) }}" method="post" id="addform"
                enctype="multipart/form-data" class="space-y-3">
                @csrf
                @method('put')

                <input type="hidden" name="nama_old" value="{{ $course->nama }}">
                <input type="hidden" name="gambar_old" value="{{ $course->gambar }}">
                    ...
                    <div class="collapse-content text-sm grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5 mt-4">
                        <div>
                            ...
                            <input type="text" class="input w-full" placeholder="Type here" name="nama"
                                value="{{ old('nama', $course->nama) }}" onkeyup="upCase(this)" autocomplete="off"
                        ...
                        <div>
                            ...
                            <input type="datetime-local" class="input w-full" placeholder="Type here" name="start_date"
                                value="{{ old('start_date', $course->start_date) }}" onkeyup="upCase(this)"
                        ...
                        <div>
                            ...
                            <input type="datetime-local" class="input w-full" placeholder="Type here" name="end_date"
                                value="{{ old('end_date', $course->end_date) }}" onkeyup="upCase(this)" autocomplete="off"
                                id="end_date" />
                            @error('end_date')
                                <p class="mt-2 text-red-500">{{ $message }}</p>
                            @enderror
                            ...
                            <select class="select w-full" name="major_id" id="major_id">
                                ...
                                @foreach ($majors as $jurusan)
                                    <option value="{{ $jurusan->id }}"
                                        {{ old('major_id', $course->major_id) == $jurusan->id ? 'selected' : '' }}>
                                        {{ $jurusan->nama }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        {{-- <div>
                            ...
                            <input type="text" class="input w-full" placeholder="Number of section" name="section"
                                value="{{ old('section', $section) }}" onkeyup="onlyNumbers(this)" autocomplete="off"
                    ...
                    <div class="collapse-content text-sm grid grid-cols-1 gap-5 mt-4">
                        <div>
                            ...
                            <input id="deskripsi" type="hidden" name="deskripsi"
                                value="{{ old('deskripsi', $course->deskripsi) }}">
                        ...
                        <div>
                            ...
                            <div class="relative border-2 border-gray-300 overflow-hidden group w-80 h-80">
                                <img src="{{ asset($course->gambar) }}" id="profileImage" alt="{{ $course->nama }}"
                                    class="w-full h-full object-cover">
                                <p class="p-4" id="text-profileImage">klik untuk memilih gambar</p>
                                <div
                                    class="absolute inset-0 bg-gsray-300/35 flex items-center justify-center opacity-0 group-hover:opacity-100 transition">
