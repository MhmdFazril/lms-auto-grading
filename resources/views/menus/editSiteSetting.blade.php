...
@section('content')
    <div class="mx-3 my-6 md:mx-auto md:max-w-7xl">
        <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] p-6">
            ...
            <form action="{{ route('dashboard.update') }}" method="post" id="addform" enctype="multipart/form-data"
                class="space-y-3">
                @csrf

                <input type="hidden" value="{{ $information->id }}" name="id">
                <input type="hidden" value="{{ $information->gambar }}" name="gambar_old">
                    ...
                    <div class="collapse-content text-sm grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5 mt-4">
                        <div>
                            ...
                            <input type="text" class="input w-full" placeholder="Masukkan Title" name="title"
                                value="{{ old('title', $information->title) }}" maxlength="100" autocomplete="off" />
                            @error('title')
                        ...
                        <div>
                            ...
                            <input type="datetime-local" class="input w-full" name="published_at"
                                value="{{ old('published_at', $information->published_at) }}" />
                            @error('published_at')
                                <p class="mt-2 text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="datetime-local" class="input w-full" name="take_down_at"
                                value="{{ old('take_down_at', $information->take_down_at) }}" />
                            @error('take_down_at')
                                <p class="mt-2 text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                        <div>
                            ...
                            <select class="select w-full" name="order" id="order">
                                <option value="1" {{ old('order', $information->order) == 1 ? 'selected' : '' }}>
                                    Urutan pertama
                                </option>
                                @foreach ($orders as $order)
                                    @if ($order->id != $information->id)
                                        <option value="{{ $order->order + 1 }}"
                                            {{ old('order', $information->order) == $order->order + 1 ? 'selected' : '' }}>
                                            Setelah {{ $order->title }}
                                        </option>
                                    @endif
                                @endforeach
                        ...
                        <div>
                            ...
                            <div class="relative border-2 border-gray-300 overflow-hidden group w-80 h-80">
                                <img src="{{ asset($information->gambar) }}" id="image" alt="Profile Picture"
                                    class="w-full h-full object-cover {{ $information->gambar ? '' : 'hidden' }}">
                                ...
                                <div
                                    ...
                                    <span id="removeImage"
                                        class="bg-red-500 text-white px-3 py-1 rounded hover:bg-red-600 transition ml-2 {{ $information->gambar ? '' : 'hidden' }}">
