...
@section('content')
    <div class="mx-3 my-6 md:mx-auto md:max-w-7xl">
        <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] p-6">
            ...
            <form action="{{ route('course.section.add-setting-post', ['course' => $course_id]) }}" method="post"
                id="addform" enctype="multipart/form-data" class="space-y-3">
                @csrf

                <div class="collapse collapse-plus bg-base-100 border border-base-300">
                    <input type="checkbox" class="dropdownAcc" checked="checked" />
                    <div class="collapse-title font-semibold text-white bg-grf-primary">Data Section</div>
                    <div class="collapse-content text-sm grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5 mt-4">
                        <div>
                            <label class="block mb-1">Nama section <x-tooltip message="required" /></label>
                            <input type="text" class="input w-full" placeholder="Type here" name="nama"
                                value="{{ old('nama') }}" autocomplete="off" id="nama" maxlength="100" />
                            @error('nama')
                ...
                <div class="flex justify-end gap-3 mt-5">
                    <a href="{{ route('course.show', ['course' => $course_id]) }}" class="btn btn-info text-white"><i
