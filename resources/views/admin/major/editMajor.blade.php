...
@section('content')
    <div class="mx-3 my-6 md:mx-auto md:max-w-7xl">
        <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] p-6">
            ...
            <form action="{{ route('major.update', ['major' => $major->id]) }}" method="post" id="addform"
                enctype="multipart/form-data" class="space-y-3">
                @csrf
                @method('put')

                <input type="hidden" name="nama_old" id="nama_old" value="{{ $major->nama }}">
                    ...
                    <div class="collapse-content text-sm grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5 mt-4">
                        <div>
                            ...
                            <input type="text" class="input w-full" placeholder="Type here" name="nama"
                                value="{{ old('nama', $major->nama) }}" onkeyup="upCase(this)" autocomplete="off"
                        ...
                        <div class="col-span-2">
                            ...
                            <textarea placeholder="Type here ...." name="deskripsi" class="textarea w-full textarea-md resize-none">{{ old('deskripsi', $major->deskripsi) }}</textarea>
                            @error('deskripsi')
                                <p class="mt-2 text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
