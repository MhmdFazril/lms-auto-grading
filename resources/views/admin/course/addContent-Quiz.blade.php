...
@section('content')
    <div class="mx-3 my-6 md:mx-auto md:max-w-7xl">
        <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] p-6">
            ...
            <form
                action="{{ route('course.content.store-quiz', ['course' => $course->id, 'courseSection' => $courseSection->id]) }}"
                    ...
                    <div class="collapse-content text-sm grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5 mt-4">
                        <div>
                            ...
                            <input type="datetime-local" class="input w-full" placeholder="Type here" name="open_quiz"
                                value="{{ old('open_quiz') }}" onkeyup="upCase(this)" autocomplete="off" id="open_quiz" />
                            @error('open_quiz')
                                <p class="mt-2 text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="datetime-local" class="input w-full" placeholder="Type here" name="close_quiz"
                                value="{{ old('close_quiz') }}" onkeyup="upCase(this)" autocomplete="off" id="close_quiz" />
                            @error('close_quiz')
                                <p class="mt-2 text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-4 gap-2">
                                <input type="text" class="input w-full" name="time_limit" id="time_limit"
                                    value="{{ old('time_limit') }}" autocomplete="off" onkeyup="onlyNumbers(this)"
                                    maxlength="15" />
